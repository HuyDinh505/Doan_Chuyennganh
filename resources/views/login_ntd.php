...
$txt_erro = "";  // Khởi tạo biến thông báo lỗi

if ((isset($_POST['dangnhap'])) && ($_POST['dangnhap'])) {
    $email = $_POST['email'];
    $pass = $_POST['password'];
    $hashed_pass = hash('sha256', $pass);  // Mã hóa mật khẩu

    $UserType = checkuser($email, $pass);
    ...
    if ($UserType != null) {
        ...
        if ($UserType == 'Admin') {
            ...
            header('location: register_ntd.php');
            exit();  // Dừng thực thi sau khi chuyển hướng
        }
    } else {
        $txt_erro = "Email hoặc mật khẩu không đúng.";  // Gán thông báo lỗi
    }
}
?>
  ...
  <body>
    ...
    <div class="container d-flex justify-content-center align-items-center">
        <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post">
            ...
            <div>
                <input type="submit" class="btn btn-success" style="width: 100%;" name="dangnhap" value="Đăng nhập">
                <?php
                    if(isset($txt_erro) && $txt_erro != ""){
                        echo '<div class="alert alert-danger mt-2">' . $txt_erro . '</div>';
                    }
                ?>
            </div>
